"""Fixtures compartidas para todos los tests de Ampara.

Los tests usan el cliente HTTP de la app directamente (ASGITransport).
Cada test genera datos propios con emails y UUIDs únicos para evitar
...
# Debe estar antes de cualquier import de la app para que el limiter lo lea.
...
async def client():
    """Cliente HTTP que habla con la app FastAPI en memoria."""
...
async def _crear_usuario(client: AsyncClient, email: str, password: str = "Test1234!") -> dict:
    ...
    assert r.status_code in (200, 201), f"_crear_usuario falló: {r.text}"
    return r.json()
...
async def _headers_operador(client: AsyncClient) -> dict:
    """Login como el primer operador/admin del sistema."""
    r = await client.post("/api/v1/auth/login", json={
        "email": "admin@ampara.pe",
        "password": "Admin2024!",
    })
    assert r.status_code == 200, f"login operador falló: {r.text}"

...
export function Sidebar({ activePage, onNavigate }: SidebarProps) {
  ...
  return (
    <aside className="w-64 min-h-screen bg-[#3B1A6B] flex flex-col shadow-xl flex-shrink-0">
      {/* Logo */}
      <div className="px-6 py-6 border-b border-white/10">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-[#8B43D4] flex items-center justify-center shadow">
      ...
      <div className="px-4 py-4 border-t border-white/10">
        <div className="flex items-center gap-3 px-2 py-2 rounded-xl">
          <div className="w-8 h-8 rounded-full bg-[#8B43D4] flex items-center justify-center flex-shrink-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-white/40 text-xs truncate capitalize">{user?.rol ?? 'operador'}</p>
          </div>
          <button
            onClick={logout}
  ...
  return (
    <button
      ...
      className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all ${
        active
          ? 'bg-[#8B43D4] text-white font-semibold shadow'

...
async def get_stats(
    ...
    estados_result = await db.execute(
        select(Denuncia.estado, func.count(Denuncia.id))
    ...
    tipos_result = await db.execute(
        select(Denuncia.tipo_violencia, func.count(Denuncia.id))
        .group_by(Denuncia.tipo_violencia)
    )
    ...
    for i in range(6, -1, -1):
        ...
        tendencia.append({"fecha": dia.isoformat(), "total": cnt_result.scalar_one()})
...
# ─── Datos para el mapa (lat/lng + metadata) ──────────────────────────────────
...
async def asignar_operador(
    ...
    op_result = await db.execute(select(Usuario).where(Usuario.id == data.operador_id, Usuario.rol.in_(["operador", "admin"])))
